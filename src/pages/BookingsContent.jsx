...
const mapStatus = (backendStatus) => {
  ...
  if (["CONFIRMATA", "ACTIV", "ACTIVE", "CONFIRMAT"].includes(s)) return "active";
  if (["ANULATA", "ANULAT", "CANCELLED", "INACTIV"].includes(s)) return "cancelled";
  if (["FINALIZATA", "COMPLETAT", "FINALIZAT", "COMPLETED"].includes(s)) return "completed";
  return "active"; 
};
...
const deduceSportName = (teren) => {
  ...
  if (nume.includes("tenis") || nume.includes("padel") || nume.includes("tennis")) return "Tenis";
...
const getSportConfig = (sportName) => {
  switch (sportName) {
    case "Fotbal": return { name: "Fotbal", icon: FaFutbol, color: "#5ED1BE", img: "https://images.unsplash.com/photo-1487466365202-1afdb86c764e?q=80&w=1173&auto=format&fit=crop" };
    case "Baschet": return { name: "Baschet", icon: FaBasketballBall, color: "#F97316", img: "https://sportarena.ro/wp/wp-content/uploads/2020/11/MG_3314-scaled.jpg" };
    case "Tenis": return { name: "Tenis", icon: GiTennisRacket, color: "#A855F7", img: "https://booksport.ro/_next/image?url=https%3A%2F%2Fimg.booksport.ro%2Fclubs%2F104%2Fsmall%2Fprimaverii_main_sala.jpg&w=1080&q=75" };
    default: return { name: "Sport", icon: FaFutbol, color: "#5ED1BE", img: "https://images.unsplash.com/photo-1487466365202-1afdb86c764e?q=80&w=1173&auto=format&fit=crop" };
  }
};
...
const PremiumBookingCard = ({ booking, onCancelClick, onAlert }) => {
  ...
  const handleDownloadInvoice = async () => {
    try {
      ...
      const paymentBaseUrl = getBaseUrl(API_URLS.PAYMENTS) || "http://localhost:8084";
      const response = await fetch(`${paymentBaseUrl}/api/payment/${booking.paymentId}/pdf`);
  ...
  return (
    <Box role="group" bg="rgba(22, 24, 28, 0.6)" backdropFilter="blur(20px)" border={DS.border} borderRadius="3xl" p={{ base: 5, md: 6 }} position="relative" transition={DS.transition} _hover={{ transform: "translateY(-4px)", borderColor: booking.color, boxShadow: `0 20px 40px -15px ${booking.color}40`, bg: "rgba(22, 24, 28, 0.85)" }}>
      ...
      <Flex justify="space-between" align="center" mb={6}>
        <Flex align="center" gap={3}>
          <Flex align="center" gap={2} bg="whiteAlpha.50" px={3} py={1.5} borderRadius="full" border="1px solid" borderColor="whiteAlpha.100">
            ...
            <Text fontSize="xs" fontWeight="800" color={DS.colors.text} letterSpacing="0.5px" textTransform="uppercase">{booking.sport}</Text>
          </Flex>
          <Text fontSize="xs" fontWeight="600" color={DS.colors.muted} display={{ base: "none", sm: "block" }}>ID REZ: {booking.id}</Text>
        </Flex>
        <Badge display="flex" alignItems="center" gap={1.5} bg={statusStyles.bg} color={statusStyles.color} px={3} py={1.5} borderRadius="full" fontSize="10px" fontWeight="900" letterSpacing="0.5px">
          <Icon as={statusStyles.icon} size={12} />{statusStyles.label}
        </Badge>
      </Flex>
      ...
      <Flex direction={{ base: "column", md: "row" }} gap={6} align={{ base: "flex-start", md: "center" }}>
        ...
        <Box flex={1}>
          ...
          <Flex align="center" gap={2} color={DS.colors.muted}><FiMapPin size={14} /><Text fontSize="sm" fontWeight="600">{booking.location}</Text></Flex>
        </Box>
      ...
      {booking.extraServices && booking.extraServices.length > 0 && (
        <Box w="full" mt={6} p={3} bg="whiteAlpha.50" borderRadius="xl" border="1px solid" borderColor="whiteAlpha.100">
          <Text fontSize="10px" color={DS.colors.muted} fontWeight="800" letterSpacing="1px" mb={2}>SERVICII EXTRA INCLUSE</Text>
          <Flex wrap="wrap" gap={2}>
            {booking.extraServices.map((extra, idx) => (
              <Badge key={idx} bg="whiteAlpha.200" border="1px solid" borderColor="whiteAlpha.200" color={DS.colors.text} px={2.5} py={1} borderRadius="lg" textTransform="none" fontWeight="600">
                {extra.nume || "Serviciu"} <Text as="span" color={DS.colors.brand} ml={1}>+{extra.pret} RON</Text>
              </Badge>
            ))}
          </Flex>
        </Box>
      ...
      <Flex justify="space-between" align="center">
        <VStack align="start" spacing={0}>
          <Text fontSize="10px" color={DS.colors.muted} fontWeight="800" letterSpacing="1px">TOTAL DE PLATĂ</Text>
          <Text fontSize="lg" color={DS.colors.text} fontWeight="900">{booking.price}</Text>
        </VStack>
        <HStack spacing={3}>
          {booking.paymentId && booking.paymentId !== "N/A" && (
            <Button h="40px" px={4} bg="whiteAlpha.50" color={DS.colors.text} borderRadius="xl" fontSize="xs" fontWeight="800" border="1px solid" borderColor="whiteAlpha.100" leftIcon={<FiDownloadCloud size={14} />} _hover={{ bg: "whiteAlpha.200", color: DS.colors.brand }} transition={DS.transition} onClick={handleDownloadInvoice} isLoading={isDownloading} loadingText="Se generează...">
              Factură
            </Button>
          )}
...
const BookingsContent = () => {
  ...
  const [customToast, setCustomToast] = useState({ isOpen: false, type: "success", message: "" });
  ...
  const fetchData = async () => {
    try {
      ...
      const paymentBaseUrl = getBaseUrl(API_URLS.PAYMENTS) || "http://localhost:8084";
      const bookingBaseUrl = getBaseUrl(API_URLS.BOOKING) || "http://localhost:8082";

      const resPayments = await fetch(`${paymentBaseUrl}/api/payment/user/${ID_USER_CURRENT}`);
      ...
      if (activeTab === "all") {
        const [resAct, resAnu, resComp] = await Promise.all([
          fetch(`${bookingBaseUrl}/api/rezervari/user/${ID_USER_CURRENT}/ACTIV`),
          fetch(`${bookingBaseUrl}/api/rezervari/user/${ID_USER_CURRENT}/ANULAT`),
          fetch(`${bookingBaseUrl}/api/rezervari/user/${ID_USER_CURRENT}/COMPLETAT`)
        ]);
        ...
        const resBookings = await fetch(`${bookingBaseUrl}/api/rezervari/user/${ID_USER_CURRENT}/${backendStatusParam}`);
      ...
      for (const pay of payments) {
        ...
        const bookingMatch = bookings.find(b => String(b.idRezervare || b.id || b.id_rezervare) === String(idRez));
        if (!bookingMatch && activeTab !== "all") continue; 

        const backendStatus = bookingMatch ? (bookingMatch.status || bookingMatch.Status) : "ACTIV";
        const sportName = deduceSportName(teren);
        const sportConfig = getSportConfig(sportName);

        let orasString = baza.oras || "Oraș necunoscut";
        if (typeof orasString === "object") orasString = orasString.nume || "Oraș";
        ...
        formattedData.push({
          ...
          time: rezervare.oraStart && rezervare.oraFinal ? `${formatTime(rezervare.oraStart)} - ${formatTime(rezervare.oraFinal)}` : "-",
  ...
  const handleConfirmCancel = async () => {
    try {
      ...
      const response = await fetch(`${bookingBaseUrl}/api/rezervari/${bookingToCancel}/anulare`, {
        method: "PATCH",
      ...
      if (response.ok) {
        ...
        showToast("error", `Nu s-a putut anula! Eroare: ${errText || 'Rezervare indisponibilă.'}`);
      }
    } catch (error) {
  ...
  return (
    <Box position="relative" minH="100vh" bg={DS.colors.canvas} overflow="hidden" mt={{ base: -6, md: -10 }} mb={{ base: "-80px", md: -10 }} mx={{ base: -4, md: -10, lg: -16 }} py={{ base: 10, md: 16 }} px={{ base: 4, md: 8 }}>
      ...
      {customToast.isOpen && (
        <Box
          position="fixed" top={{ base: "20px", md: "40px" }} right={{ base: "5%", md: "40px" }} zIndex={10000}
          w={{ base: "90%", md: "auto" }} minW="300px" 
          bg={customToast.type === "success" ? DS.colors.brand : DS.colors.danger}
          borderRadius="2xl" p={4}
          boxShadow="0 25px 50px -12px rgba(0,0,0,0.8)" display="flex" alignItems="center" gap={4}
        ...
        >
          <Flex align="center" justify="center" boxSize="40px" borderRadius="full" flexShrink={0}
            bg="blackAlpha.200" color={DS.colors.canvas} 
          >
            <Icon as={customToast.type === "success" ? FiCheckCircle : FiXCircle} boxSize={5} />
          </Flex>
          <Text color={DS.colors.canvas} fontWeight="800" fontSize="sm" lineHeight="1.4">
            {customToast.message}
          </Text>
        </Box>
      )}
      ...
      <Box position="relative" zIndex={1} maxW="900px" mx="auto" pt={{ base: 4, md: 8 }}>
        ...
        <Flex direction={{ base: "column", md: "row" }} justify="flex-start" align={{ base: "stretch", md: "center" }} mb={8} gap={4}>
          <Flex overflowX="auto" gap={3} pb={{ base: 2, md: 0 }} sx={{ "&::-webkit-scrollbar": { display: "none" }, scrollbarWidth: "none" }}>
            {TABS.map((tab) => (
              <Button key={tab.id} h="44px" px={6} borderRadius="full" bg={activeTab === tab.id ? DS.colors.text : "transparent"} color={activeTab === tab.id ? DS.colors.canvas : DS.colors.muted} border="1px solid" borderColor={activeTab === tab.id ? DS.colors.text : "whiteAlpha.200"} fontWeight="800" fontSize="sm" flexShrink={0} onClick={() => setActiveTab(tab.id)} _hover={{ bg: activeTab === tab.id ? DS.colors.text : "whiteAlpha.100", color: activeTab === tab.id ? DS.colors.canvas : DS.colors.text }} transition={DS.transition}>
                {tab.label}
              </Button>
            ))}
          </Flex>
        ...
        {loading ? (
          ...
          <Grid templateColumns="1fr" gap={6}>
            {realBookings.map((booking) => (
              <PremiumBookingCard key={booking.id} booking={booking} onCancelClick={handleOpenCancelModal} onAlert={showToast} />
            ))}
          </Grid>
        ) : (
          <Flex direction="column" align="center" justify="center" py={20} opacity={0.6}>
            ...
            <Text fontSize="sm" color={DS.colors.muted} textAlign="center">Nu s-a găsit nicio rezervare pentru acest status din backend.</Text>
          </Flex>
        )}
      </Box>
        ...
        <Box position="fixed" top={0} left={0} w="100vw" h="100vh" zIndex={9999} display="flex" alignItems="center" justifyContent="center">
          ...
          <Box position="relative" bg={DS.colors.card} border={DS.border} borderRadius="3xl" p={8} maxW="400px" w="90%" textAlign="center" boxShadow="0 25px 50px -12px rgba(0,0,0,0.9)">
            ...
            <Text fontSize="sm" color={DS.colors.muted} mb={8}>
              Ești sigur că vrei să anulezi meciul? Această acțiune este ireversibilă.
            </Text>
            <HStack spacing={4}>
