...
const initialMockData = [
  ...
  },
  {
    ...
    Teren: {
      numeTeren: "Teren Fotbal Sintetic #3",
      numarLocuri: 12,
      pretPeOra: 100.0,
    },
    extraServicii: [],
...
// ==========================================
// 4. CARD REZERVARE (CU CONFIRMARE INLINE)
// ==========================================
const RezervareCard = ({ data, onCancel }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [confirmCancel, setConfirmCancel] = useState(false); // Stare pentru UI de confirmare anulare
      ...
      {isExpanded && (
        ...
        >
          <Grid templateColumns={{ base: "1fr", lg: "2fr 1fr" }} gap={10}>
            ...
            {/* Partea Dreaptă: Date Client & Acțiuni */}
            <VStack
              align="stretch"
              spacing={4}
              bg="whiteAlpha.50"
...
// ==========================================
// 5. PAGINA PRINCIPALĂ (STATE & FILTRARE)
// ==========================================
const RezervariBaza = () => {
  const [rezervari, setRezervari] = useState(initialMockData);
  const [filtruTeren, setFiltruTeren] = useState("TOATE");

  // Extragem dinamic toate numele de terenuri unice
  const terenuriUnice = useMemo(() => {
    const toateTerenurile = rezervari.map((r) => r.Teren.numeTeren);
    return ["TOATE", ...new Set(toateTerenurile)];
  }, [rezervari]);

  // Aplicăm filtrul
  const rezervariFiltrate = useMemo(() => {
    if (filtruTeren === "TOATE") return rezervari;
    return rezervari.filter((r) => r.Teren.numeTeren === filtruTeren);
  }, [rezervari, filtruTeren]);

  // Funcția "apel de API" care anulează
  const anuleazaRezervare = (idPayment) => {
    // Într-un proiect real aici faci: await fetch(`/api/rezervari/cancel/${idPayment}`, { method: 'PUT' })
    setRezervari((prev) =>
      prev.map((rez) =>
    ...
    <Box>
      <VStack align="start" spacing={2} mb={8}>
        ...
        <Text color="gray.400" fontSize="lg">
          Monitorizează statusul rezervărilor, încasările și facturile
          clienților.
        </Text>
      </VStack>

      {/* --- FILTRARE TERENURI (PILLS MODERNE) --- */}
      <Flex
        mb={8}
        gap={3}
        overflowX="auto"
      ...
      >
        <HStack color="gray.500" mr={2}>
          <Icon as={FiFilter} />
          <Text fontSize="sm" fontWeight="bold" textTransform="uppercase">
            Filtru:
          </Text>
        </HStack>
        {terenuriUnice.map((teren) => (
          <Button
            key={teren}
            size="sm"
            borderRadius="full"
            bg={filtruTeren === teren ? colors.accent : "whiteAlpha.100"}
            color={filtruTeren === teren ? "black" : "gray.300"}
            _hover={{
              bg: filtruTeren === teren ? colors.accent : "whiteAlpha.300",
            }}
            onClick={() => setFiltruTeren(teren)}
            whiteSpace="nowrap"
            px={5}
          >
            {teren}
      ...
      <VStack spacing={5} align="stretch">
        {rezervariFiltrate.length > 0 ? (
          ...
          >
            <Text color="gray.400">
              Nu am găsit nicio rezervare pentru terenul selectat.
            </Text>
          </Box>
        )}
      </VStack>
